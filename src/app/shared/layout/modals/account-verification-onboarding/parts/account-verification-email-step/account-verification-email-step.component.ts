...
@Component({
  selector: 'app-account-verification-email-step',
  templateUrl: './account-verification-email-step.component.html',
  styleUrl: './account-verification-email-step.component.scss'
})
export class AccountVerificationEmailStepComponent {
  ...
  private verificationValue: string = '';
  ...
  get title() {
    ...
    return `Digite o código do ${verificationTypeName}`;
  }

  get description() {
    return this.verificationTypeIsEmail
      ? `Um e-mail com o código de verificação foi enviada para <strong class="color-primary">${this.verificationValue}</strong>`
      : `Uma mensagem de texto com o código de verificação foi <strong class="color-primary">${this.verificationValue}</strong>`
  }
