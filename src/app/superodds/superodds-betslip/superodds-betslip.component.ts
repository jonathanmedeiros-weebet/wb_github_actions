...
@Component({
    selector: 'app-superodds-betslip',
    templateUrl: './superodds-betslip.component.html',
    styleUrls: ['./superodds-betslip.component.css']
})
export class SuperoddsBetslipComponent extends BaseFormComponent implements OnInit, OnDestroy {
    ...
    removerItem(index) {
        ...
        this.superoddService.atualizarItens(this.itens.value);
    }
    ...
    calcularPossibilidadeGanho(valor) {
        ...
        this.itens.value.forEach(item => {
            cotacao *= item.odd.value;
        });

        const premio = valor * cotacao;
        this.possibilidadeGanho = premio;
    }
    ...
    async submit() {
        if (!this.isCliente && !this.modoCambista) {
            ...
            if (this.itens.length > 1) {
                valido = false;
                msg = `Por favor, inclua no MÁXIMO 1 evento por bilhete.`;
            }
    ...
    apostaSuccess(aposta) {
        ...
        this.superoddService.atualizarItens([]);
        this.form.reset();
    ...
    salvarAposta(dados) {
        this.superoddService.createBet(dados)
            .subscribe(
    ...
    finalizarApostaDeslogado() {
        ...
        if (this.tipoApostaDeslogado === 'preaposta') {
            this.superoddService.createPreBet(values)
                .pipe(takeUntil(this.unsub$))
                .subscribe(
