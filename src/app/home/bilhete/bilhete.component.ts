...
export class BilheteComponent extends BaseFormComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        const {
            ...
            valor_min_aposta,
            valor_max_aposta
        } = this.paramsService.getOpcoes();

        this.mobileScreen = window.innerWidth <= 1024;
        this.modoCambista = modo_cambista;
        this.apostaMinima = valor_min_aposta;
        this.apostaMaximo = valor_max_aposta;

        this.createForm();
        this.opcoes = this.paramsService.getOpcoes();
        this.setDelay();

        this.auth.logado.subscribe((isLoggedIn: boolean) => {
            this.isLoggedIn = isLoggedIn;
            if (isLoggedIn) {
                this.auth.cliente.subscribe((isCliente: boolean) => {
    ...
    toggleBilhete() {
        this.showBilhete = !this.showBilhete;
    }
    ...
    createForm() {
        this.form = this.fb.group({
            ...
            valor: [null, [Validators.required, Validators.min(this.apostaMinima), Validators.max(this.apostaMaximo)]],
            itens: this.fb.array([]),
